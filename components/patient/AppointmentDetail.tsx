"use client";
import { RefreshCcw, Trash2, Clock } from "lucide-react";

interface AppointmentDetailProps {
  appointment: {
    id: string;
    date: string;
    time: string;
    doctor: string;
    status: string;
    type: string;
    location: string;
    notes: string;
  };
  onBack: () => void;
}

export default function AppointmentDetail({ appointment, onBack }: AppointmentDetailProps) {
  return (
    <div className="p-4">
      <button onClick={onBack} className="text-blue-500 mb-4 flex items-center gap-1">
        ← Retour à mes rendez-vous
      </button>

      <div className="bg-white shadow rounded-2xl overflow-hidden">
        <div className="bg-blue-50 p-4">
          <h2 className="text-lg font-semibold text-blue-700">
            {appointment.date} à {appointment.time}
          </h2>
          <p className="text-sm text-blue-500">{appointment.type}</p>
        </div>

        <div className="p-4 space-y-2">
          <p className="text-gray-700 flex items-center gap-2">
            <span className="font-medium">Médecin :</span> {appointment.doctor}
          </p>
          <p className="text-gray-700 flex items-center gap-2">
            <span className="font-medium">Lieu :</span> {appointment.location}
          </p>
          <p className="text-gray-500 italic">“{appointment.notes}”</p>

          <div className="flex gap-2 flex-wrap mt-4">
            <button className="flex items-center gap-1 bg-blue-500 text-white px-3 py-2 rounded-lg hover:bg-blue-600">
              <RefreshCcw className="w-4 h-4" /> Replanifier
            </button>
            <button className="flex items-center gap-1 bg-red-500 text-white px-3 py-2 rounded-lg hover:bg-red-600">
              <Trash2 className="w-4 h-4" /> Annuler
            </button>
            <button className="flex items-center gap-1 bg-green-500 text-white px-3 py-2 rounded-lg hover:bg-green-600">
              <Clock className="w-4 h-4" /> Rappel
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
