"use client";
import OrderItem from "./OrderItem";
import { Truck, RefreshCcw, Repeat } from "lucide-react";

interface OrderDetailProps {
  order: {
    id: string;
    date: string;
    status: string;
    items: { name: string; quantity: number }[];
    total: number;
    pharmacy: string;
  };
  onBack: () => void;
}

export default function OrderDetail({ order, onBack }: OrderDetailProps) {
  return (
    <div className="p-4">
      <button onClick={onBack} className="text-green-600 mb-4 flex items-center gap-1">
        ← Retour à mes commandes
      </button>

      <div className="bg-white shadow rounded-2xl overflow-hidden">
        <div className="bg-green-50 p-4">
          <h2 className="text-lg font-semibold text-green-700">Commande du {order.date}</h2>
          <p className="text-sm text-green-500">Pharmacie : {order.pharmacy}</p>
          <span className="inline-block mt-2 px-3 py-1 rounded-full bg-green-100 text-green-700 text-xs font-medium">
            Statut : {order.status}
          </span>
        </div>

        <div className="p-4 space-y-2">
          {order.items.map((item, idx) => (
            <div key={idx} className="bg-gray-50 p-3 rounded-lg">
              <OrderItem {...item} />
            </div>
          ))}

          <div className="text-right font-bold text-lg text-green-700 mt-4">
            Total : ${order.total.toFixed(2)}
          </div>

          <div className="flex gap-2 flex-wrap mt-4">
            <button className="flex items-center gap-1 bg-red-500 text-white px-3 py-2 rounded-lg hover:bg-red-600">
              <Truck className="w-4 h-4" /> Suivre
            </button>
            <button className="flex items-center gap-1 bg-blue-500 text-white px-3 py-2 rounded-lg hover:bg-blue-600">
              <RefreshCcw className="w-4 h-4" /> Annuler
            </button>
            <button className="flex items-center gap-1 bg-green-500 text-white px-3 py-2 rounded-lg hover:bg-green-600">
              <Repeat className="w-4 h-4" /> Recommander
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
